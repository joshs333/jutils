 ...
 * @brief implementation of jutils::shared_ptr an interopable shared_ptr data structure
 * @date 2022-05-07
 */
#ifndef JCONTROLS_UTIL_SHARED_PTR_HPP_
#define JCONTROLS_UTIL_SHARED_PTR_HPP_
...
class shared_ptr {
    ...
    struct control_block {
         ...
         **/
        control_block(MemType pointer_location, T* unowned_pointer):
            pointer(nullptr),
            use_count(1),
            observer_count(1),
        ...
        {
            if(location == MemType::DEVICE) {
                device_pointer = unowned_pointer;
            } else {
                if(location == MemType::UNIFIED) {
                    device_pointer = unowned_pointer;
                }
...
public:
     ...
     */
	shared_ptr():
        block_(new control_block(MemType::HOST, nullptr)),
	...
	shared_ptr(T* pointer):
        block_(new control_block(MemType::HOST, pointer)),
        device_pointer_(block_->device_pointer)
    {}
     ...
     * @param pointer the unowned pointer to take ownership of
     */
	shared_ptr(jutils::MemType location, T* pointer):
    ...
    shared_ptr(shared_ptr&& obj) {
        block_ = obj.block_;
        obj.block_ = new control_block(MemType::HOST, nullptr);
        device_pointer_ = block_->device_pointer;
	}
    ...
    void toGPU() {
            ...
            if (block_->location == MemType::DEVICE) {
                ...
                if constexpr(shared_ptr_helpers::toGPUDefined<T>{})
                    copy = block_->pointer->toGPU(block_->device_pointer);
                ...
                device_pointer_ = block_->device_pointer;
            } else if(block_->location == MemType::UNIFIED) {
                if constexpr(shared_ptr_helpers::toGPUDefined<T>{})
 ...
 */
template<typename T, typename... Args>
inline shared_ptr<T> make_shared_on(jutils::MemType location, Args&&... args) {
    // NOTE: be careful to keep this construction logic in sync
    // with shared_ptr<T>::control_block::dereference() destruction logic
    if(location == jutils::MemType::HOST) {
        return shared_ptr<T>(jutils::MemType::HOST, new T(args...));
    } else if(location == jutils::MemType::UNIFIED) {
        #if CUDA_AVAILABLE
            T* pointer;
            cudaMallocManaged(&pointer, sizeof(T));
            return shared_ptr<T>(jutils::MemType::UNIFIED, new(pointer) T(args...));
        #else
...
inline shared_ptr<T> make_shared(Args&&... args) {
    return make_shared_on<T>(jutils::MemType::HOST, args...);
}
...
inline shared_ptr<T> make_shared_on_unified(Args&&... args) {
    return make_shared_on<T>(jutils::MemType::UNIFIED, args...);
}

} /* namespace jutils */

#endif /* JCONTROLS_UTIL_SHARED_PTR_HPP_ */
